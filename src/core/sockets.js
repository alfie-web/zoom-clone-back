...
// 			socket.join(roomId);
// 			socketList[socket.id] = { userName, video: true, audio: true, userId };
...
// 					clients.forEach((client) => {
// 						// Add User List
// 						users.push({ socketId: client, info: socketList[client] });
// 					});
...
// 			socket.broadcast
// 				.to(roomId)
// 				.emit('FE-user-leave', { socketId: socket.id, userName: [socket.id] });
...
// 			socket.broadcast
// 				.to(roomId)
// 				.emit('FE-toggle-camera', { socketId: socket.id, switchTarget });
// 		});
// 	});
